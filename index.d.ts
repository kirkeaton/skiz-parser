...
export interface SkizTrackBatteryUsage {
  level: number;
  status: string;
  timestamp: Date;
}

export interface SkizTrackRelativeAltitude {
  pressure: number;
...
export interface SkizTrack {
  activity: string;
  batteryUsage: SkizTrackBatteryUsage[];
  conditions: string;
  description: string;
  duration: number;
  finish: Date;
  ...
  platform: string;
  rating: number;
  relativeAltitude: SkizTrackRelativeAltitude[];
  start: Date;
